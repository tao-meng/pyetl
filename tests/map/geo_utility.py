...
def distance_to_road(location, points):
    location = tuple(map(lambda x: float(x), location))
    # 约100km
    dd_min = 1
...
def roads_near_point(location, all_road):
    ...
    for id, points in all_road.items():
        ...
        #         min(m[1], n[1]) - offset < p[1] < max(m[1], n[1]) + offset:
        # p, m, n = location, (min(x), min(y)), (max(x), max(y))
        # if (m[0] - offset < p[0] < n[0] + offset and
        #         m[1] - offset < p[1] < n[1] + offset):
        p = location
        if (min(points, key=lambda x: x[0])[0] - offset < p[0] < max(points, key=lambda x: x[0])[0] + offset and
                min(points, key=lambda x: x[1])[1] - offset < p[1] < max(points, key=lambda x: x[1])[1] + offset):
...
def point_link_road(location, near_roads):
    ...
    for i, j in near_roads.items():
        ...
        for ii in range(len(points) - 1):
            ...
            dd_min = min(dd, dd_min)
        roads[i] = dd_min
    road_min = min(roads.items(), key=lambda x: x[1])
    if road_min[1] < 0.001:
        road_id = road_min[0]
        return road_id
...
def min_index_list(d, d_min):
    ...
    if min_index == len(d) - 1:
        ...
        rs = min_index_list(
            d[min_index + 1:], d_min) if min(d[min_index + 1:]) == d_min else None
    if rs:
...
def road_belong_to(unique_id, colume_polygon, colume_belong, table_name, name_type='code'):
    ...
    with connection(oracle_uri) as db:
        rs = db.query('select %s,polygon from VST_ENTITY_PATROL_AREA' % name_type)
        ...
        sql = "update %s set %s=:1 where %s=:2" % (table_name, colume_belong, unique_id)
...
def location_belong_to(unique_id, colume_lon, colume_lat, colume_belong, table_name, name_type='code'):
    ...
    with connection(oracle_uri) as db:
        rs = db.query('select %s,polygon from VST_ENTITY_PATROL_AREA' % name_type)
        sql = 'select %s,%s,%s from %s' % (unique_id, colume_lon, colume_lat, table_name)
        acdnts = db.query(sql)
        dict_section_id_to_area = {}
...
def main():
    ...
    """
    with connection(oracle_uri) as db:
        rs_road = db.query("select section_id,polygon from %s " % 'VST_ENTITY_ROAD_SECTION')
        all_road = {}
        for i, j in rs_road:
            all_road[i] = str_to_points(j)
        rs_points = db.query(
            "select ID,x,y from VST_ENTITY_ACCIDENT where x is not null")
        args = []
        for i, x, y in rs_points:
            ...
            if near_roads:
                ...
                args.append([result, i]) if result is not None else print(i, y, ',', x)
        # db.creat_table('sgld', ['acdnt_id', 'link_id'])
        sql = 'update VST_ENTITY_ACCIDENT set road_section_id=:1 where id=:1'
        db.insert_or_update(sql, args)
...
if __name__ == '__main__':
    ...
    # cProfile.run('main()')
    main()
    # road_belong_to(unique_id='id', colume_polygon='POLYGON', colume_belong='BELONG_TO', table_name='VST_ROAD_SECTION', name_type='patrol_id')
    # location_belong_to(unique_id='ID', colume_lon='X', colume_lat='Y', colume_belong='BELONG_TO', table_name='VST_YJ_ACCIDENT', name_type='code')
